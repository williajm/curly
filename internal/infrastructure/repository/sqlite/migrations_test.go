...
func TestLoadMigrations_SkipsNonSQLFiles(t *testing.T) {
	...
	for _, f := range files {
		...
		err := os.WriteFile(path, []byte(f.content), 0600)
		require.NoError(t, err)
	}
...
func TestLoadMigrations_SkipsDirectories(t *testing.T) {
	...
	err := os.WriteFile(migrationPath, []byte("CREATE TABLE test (id INTEGER);"), 0600)
	require.NoError(t, err)
...
func TestParseMigrationFile_InvalidFormat(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := os.WriteFile(filePath, []byte("CREATE TABLE test (id INTEGER);"), 0600)
			require.NoError(t, err)
...
func TestRunMigrations_AppliesOnlyPending(t *testing.T) {
	...
	err = os.WriteFile(newMigration, []byte("CREATE TABLE table4 (id INTEGER);"), 0600)
	require.NoError(t, err)
